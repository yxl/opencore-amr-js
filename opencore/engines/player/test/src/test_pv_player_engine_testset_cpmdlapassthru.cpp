...
void pvplayer_async_test_cpmdlapassthru::Run()
{
    int error = 0;
    ...
    switch (iState)
    {
        ...
        break;
        case STATE_QUERYLICENSEACQIF:
        {
            // Request the license acquisition interface.
            PVUuid uuid(PVPlayerLicenseAcquisitionInterfaceUuid);
            OSCL_TRY(error, iCurrentCmdId = iPlayer->QueryInterface(uuid, (PVInterface*&)iLicenseIF, (OsclAny*) & iContextObject));
            OSCL_FIRST_CATCH_ANY(error, PVPATB_TEST_IS_TRUE(false); iState = STATE_CLEANUPANDCOMPLETE; RunIfNotReady());
        }
        break;
        ...
        {
            fprintf(iTestMsgOutputFile, "Acquiring License...");
            OSCL_TRY(error, iAcquireLicenseCmdId = iLicenseIF->AcquireLicense(
                                                       NULL , 0 //DLA data
            ...
            {
                iState = STATE_CANCEL_ACQUIRELICENSE;
                RunIfNotReady(600*1000);
            }
        }
        break;

        case STATE_CANCEL_ACQUIRELICENSE:
        {
        ...
        case STATE_INIT2:
        {
            fprintf(iTestMsgOutputFile, "***Initializing2...\n");
            OSCL_TRY(error, iCurrentCmdId = iPlayer->Init((OsclAny*) & iContextObject));
            OSCL_FIRST_CATCH_ANY(error, PVPATB_TEST_IS_TRUE(false); iState = STATE_CLEANUPANDCOMPLETE; RunIfNotReady());
        }
        break;
        ...
        case STATE_ADDDATASINK_VIDEO:
        {
            ...
            {
                videoSinkFileName += _STRLIT_WCHAR("test_player_cpmdlapassthru_");
        ...
        case STATE_ADDDATASINK_AUDIO:
        {
            ...
            {
                audioSinkFileName += _STRLIT_WCHAR("test_cpmdlapassthru_");
                OSCL_wHeapString<OsclMemAllocator> inputfilename;
                RetrieveFilename(wFileName.get_str(), inputfilename);
...
void pvplayer_async_test_cpmdlapassthru::CommandCompleted(const PVCmdResponse& aResponse)
{
    if (aResponse.GetCmdId() != iCurrentCmdId && aResponse.GetCmdId() != iCancelCmdId
            && aResponse.GetCmdId() != iAcquireLicenseCmdId)
    {
        // Wrong command ID.
        PVPATB_TEST_IS_TRUE(false);
        iState = STATE_CLEANUPANDCOMPLETE;
    ...
    //Handling for STATE_ACQUIRELICENSE, we might have overwritten iState variable for "Cancel" TestCase
    //So cannot depend on iState to know which Command's command has completed now
    if ((aResponse.GetCmdId() == iAcquireLicenseCmdId) &&
            ((iState == STATE_ACQUIRELICENSE) || (iState == STATE_CANCEL_ACQUIRELICENSE)))//If this is AcquireLic commandCompleted
    {
        if (aResponse.GetCmdStatus() == PVMFSuccess)
        {
            // Got the license.
            fprintf(iTestMsgOutputFile, "\nAcquire License...Success\n");
            if (!oCancelAcquireLicense)
            {
                // Call Init again to start playback
                iState = STATE_INIT2;
                RunIfNotReady();
            }
            else
            {
                //Now fire cancelAcquireLic, it shud fail
                if (iTestMode == OMA1_DLA_CANCEL_ACQUIRE_LICENSE_FAILS)
                {
                    iState = STATE_CANCEL_ACQUIRELICENSE;
                    RunIfNotReady();
                }
    ...
    switch (iState)
    {
        ...
        case STATE_CANCEL_ACQUIRELICENSE:
            if (aResponse.GetCmdStatus() == PVMFSuccess)
            {
                // AcquireLicense is canceled
                fprintf(iTestMsgOutputFile, "...Cancel AcquireLicense Success\n");
                iState = STATE_CLEANUPANDCOMPLETE;
                RunIfNotReady();
            }
            else if (aResponse.GetCmdStatus() == PVMFErrCancelled)
            ...
            {
                // AcquireLicense or CancelAcquireLicense failed
                fprintf(iTestMsgOutputFile, "Cancel AcquireLicense ...Failed!\n");
                //Means CPM_DLA_PASSTRHU_CancelAcquireLicenseTooLate_CancelFails
                if (iTestMode == OMA1_DLA_CANCEL_ACQUIRE_LICENSE_FAILS)
                {
                    //This is not a failure, Cancel was fired too late, AcqLic has already succeeded
                    iState = STATE_INIT2;
                    RunIfNotReady();
                }
                else
...
void pvplayer_async_test_cpmdlapassthru::HandleErrorEvent(const PVAsyncErrorEvent& aEvent)
{
...
void pvplayer_async_test_cpmdlapassthru::HandleRTSPNodeErrors(int32 aErr)
{
...
void pvplayer_async_test_cpmdlapassthru::HandleStreamingManagerNodeErrors(int32 aErr)
{
...
void pvplayer_async_test_cpmdlapassthru::HandleJitterBufferNodeErrors(int32 aErr)
{
...
void pvplayer_async_test_cpmdlapassthru::HandleMediaLayerNodeErrors(int32 aErr)
{
...
void pvplayer_async_test_cpmdlapassthru::HandleProtocolEngineNodeErrors(int32 aErr)
{
        ...
        case PVMFInfoPositionStatus:
        {
            ...
            if (iface->queryInterface(infomsguuid, (PVInterface*&)infomsgiface) == true)
            {
                ...
                if ((infouuid == PVPlayerErrorInfoEventTypesUUID) && (infocode == PVPlayerInfoPlaybackPositionStatus))
                {
                    ...
                    fprintf(iTestMsgOutputFile, "Position:%d\n", aPos2);
                }
            }
        }
        break;
...
void pvplayer_async_test_cpmdlapassthru::PrintMetadataInfo()
{
    uint32 i = 0;
